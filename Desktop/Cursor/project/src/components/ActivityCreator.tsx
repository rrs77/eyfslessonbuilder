        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Year Groups - Multiple Selection */}
              <div>
                <label className="block text-xl font-bold text-gray-900 mb-4 flex items-center space-x-2">
                  <span className="w-2 h-2 bg-indigo-600 rounded-full"></span>
                  <span>Year Groups</span>
                </label>
                <p className="text-sm text-gray-600 mb-4">Select all applicable year groups for this activity:</p>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {simplifiedLevels.map(yearGroup => {
                    const isSelected = activity.yearGroups.includes(yearGroup);
                    return (
                      <label
                        key={yearGroup}
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setActivity(prev => ({
                                ...prev,
                                yearGroups: [...prev.yearGroups, yearGroup]
                              }));
                            } else {
                              setActivity(prev => ({
                                ...prev,
                                yearGroups: prev.yearGroups.filter(yg => yg !== yearGroup)
                              }));
                            }
                          }}
