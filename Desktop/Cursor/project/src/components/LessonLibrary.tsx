...
export function LessonLibrary({ onLessonSelect, className = '', onAssignToUnit, onEditLesson }: LessonLibraryProps) {
  const { lessonNumbers, lessons, currentSheetInfo, halfTerms, getLessonsForHalfTerm } = useData();
  const { getThemeForClass } = useSettings();
  
  // Use static HALF_TERMS for the modal (with proper names and months)
  // but merge with dynamic data from context for current assignments
  const halfTermsForModal = HALF_TERMS.map(staticHalfTerm => {
    const contextHalfTerm = halfTerms.find(ht => ht.id === staticHalfTerm.id);
    return {
      ...staticHalfTerm,
      lessons: contextHalfTerm?.lessons || [],
      isComplete: contextHalfTerm?.isComplete || false
    };
  });
  ...
  const filteredAndSortedLessons = useMemo(() => {
    let filtered = lessonNumbers.filter(lessonNum => {
      const lessonData = lessons[lessonNum];
    ...
    // Sort lessons
    filtered.sort((a, b) => {
      const lessonA = lessons[a];
      const lessonB = lessons[b];
    ...
    return filtered;
  }, [lessonNumbers, lessons, searchQuery, selectedHalfTerm, sortBy, sortOrder, halfTerms]);
