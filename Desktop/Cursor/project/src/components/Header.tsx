...
export function Header() {
  ...
  const { classes, userClasses } = useClass();
  ...
  // Filter classes based on user assignments (admin sees all, others see only assigned)
  const getAvailableClasses = () => {
    if (!user) return [];
    
    // Admin users see all classes
    if (user.email === 'rob.reichstorer@gmail.com' || user.role === 'administrator') {
      return classes;
    }
    
    // Other users see only their assigned classes
    const assignedClassIds = userClasses.map(uc => uc.class.id);
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    value={currentSheetInfo.sheet}
                    onChange={(e) => {
                      const selected = classes.find(cls => cls.id === e.target.value);
                      if (selected) {
                        setCurrentSheetInfo({
                          sheet: selected.id,
                          display: selected.displayName,
