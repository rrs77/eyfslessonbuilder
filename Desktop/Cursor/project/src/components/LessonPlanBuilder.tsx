...
import type { Activity, LessonPlan } from '../types';
...
export function LessonPlanBuilder({ 
  ...
  const { currentSheetInfo, lessons, addOrUpdateUserLessonPlan, lessonPlans, activities } = useData();
  const { getCategoryColor } = useSettings();
  
  // Get unique categories from activities (same logic as ActivityLibrary)
  ...
  const [currentLessonPlan, setCurrentLessonPlan] = useState<LessonPlan>(() => {
    ...
    if (editingLessonNumber && lessons && lessons[editingLessonNumber]) {
      const existingLesson = lessons[editingLessonNumber];
  ...
  }, [lessons, currentSheetInfo?.sheet || 'Unknown', editingLessonNumber]);
  ...
  const generateNextLessonNumber = () => {
    ...
    const lessonNumbers = Object.keys(lessons).map(num => parseInt(num));
  ...
  const handleUpdateLessonPlan = async (updatedPlan: LessonPlan) => {
    try {
      ...
      // Remove the isEditingExisting property before saving
      const { isEditingExisting, ...planToSave } = updatedPlan;
      
      // Save the lesson plan using the context function
      await addOrUpdateUserLessonPlan(planToSave);
  ...
  const handleSaveLessonPlan = async () => {
    ...
    if (!currentLessonPlan.lessonNumber) {
      ...
      setCurrentLessonPlan(updatedPlan);
      const success = await handleUpdateLessonPlan(updatedPlan);
      if (success) {
        setHasUnsavedChanges(false);
      ...
      // Remove the isEditingExisting property before saving
      const { isEditingExisting, ...planToSave } = currentLessonPlan;
      const success = await handleUpdateLessonPlan(planToSave);
      if (success) {
        setHasUnsavedChanges(false);
        if (currentLessonPlan.isEditingExisting && onEditComplete) {
          onEditComplete();
  ...
  const handleCreateNewAfterSave = async () => {
    ...
    const { isEditingExisting, ...planToSave } = {
      ...currentLessonPlan,
  ...
  const navigateToLesson = (direction: 'prev' | 'next') => {
    ...
    const classLessonPlans = lessonPlans
      .filter(plan => plan.className === currentSheetInfo?.sheet || 'Unknown')
      .sort((a, b) => {
  ...
  const filteredAndSortedActivities = React.useMemo(() => {
    ...
    return filtered;
  }, [activities, searchQuery, selectedCategory, selectedLevel, sortBy, sortOrder]);
